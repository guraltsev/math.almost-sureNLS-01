...
% Annales de l’Institut Henri Poincaré. Analyse Non Linéaire (AIHPC).
%------
% Before you edit this file, please read the
% INSTRUCTIONS given in AIHPC_instructions.pdf
% at https://ems.press/journals/aihpc/submit
%------
\documentclass{article}
\usepackage[lang = british]{ems-aihpc} %% change to `american' if you use American English
...
\titlemark{Almost sure local well-posedness for cubic NLS}
...
%------
\emsauthor{1}{Jean-Baptiste Casteras}{J-B.~Casteras}
\emsauthor{2}{Juraj F\" oldes}{J.~F\" oldes}
\emsauthor{3}{Gennady Uraltsev}{G.~Uraltsev}
...
%------
\emsaffil{1}{CMAFcIO, Faculdade de Ci\^encias da Universidade de Lisboa, Edificio C6, Piso 1, Campo Grande 1749-016 Lisboa, Portugal \email{jeanbaptiste.casteras@gmail.com}}
\emsaffil{1}{Dept. of Mathematics, University of Virginia, Kerchof Hall, Charlottesville, VA 22904-4137 \email{foldes@virginia.edu}}
\emsaffil{1}{Dept. of Mathematics, University of Virginia, Kerchof Hall, Charlottesville, VA 22904-4137 \email{gennady.uraltsev@gmail.com}}

%------
% Add MSC 2020 codes according to www.ams.org/msc/msc2020.html.
% Secondary codes (in square brackets) are optional.
%------
\classification{35Q41, 37L50}
...
\end{equation*}
so that \(h_{N}(\xi)\Delta_{N}\phi(|\xi|)=\Delta_{N}\phi(|\xi|)\). Our claim follows from \\%%!
\(\|Tf\|_{L_{e}^{2,\infty} (I\times \R^d)}\lesssim N^{X_{1}} \|f\|_{L^{2}_{x}(\R^{d})}\), 
...
\begin{equation*}|K_{N}(t,x_1 ,x')|\lesssim 
\frac{1}{\langle N x_1 \rangle^{2}}  \int_{|\xi|\lesssim2N} d\xi \lesssim
  \frac{N^d}{\langle N x_1 \rangle^{2}}.\end{equation*}
...
\begin{equation*}\left|\int_{\R} \sup_{t\in \R , x^\prime \in \R^{d-1}} |K_{N} (t,x_1, x')| dx_1 \right| \lesssim N^{d-1} 
\end{equation*}
...
\frac{d}{d \theta}\big(p_{\xi^\prime }^{-1} (\theta)\big) \right|^2  d\xi' d\theta. \end{equation*}
Returning to the original variable \(\xi_{1}\) with \(\theta=p_{\xi'}(\xi_{1})\) and using \\ %%!
\(|\frac{\partial}{\partial \theta}(p_{\xi^\prime }^{-1} (\theta)) | = \frac{1}{|\frac{d}{d \xi_1}(p_{\xi'}(\xi_{1})) |}\) we obtain 
\begin{equation*}
...
Next, we prove  \eqref{lemmaIKe1}. This time, we interpolate between the frequency limited \\%%!
Strichartz estimate \eqref{IK2} and the anisotropic estimate \eqref{IK1}. Assuming that $q \geq p$, then it holds that $q \geq 4$ and for any $h\in L^\infty (\R^d )$ and almost any fixed \(x_{1}\) we have
\begin{equation*}
...
\end{equation*}
with
\begin{equation*}\|P_N h\|_{G_N (I)} =\inf_{P_N h = h_1 +h_2} 
...
\begin{equation*} \sum_{l=1}^d\Big\|\int_0^t e^{-i(t-s) P} P_N h(s) ds \Big\|_{L_{e_l}^{\tilde{p},\tilde{q}} (I\times \R^d)}\lesssim 
N^{\alpha(\tilde{p},\tilde{q}) + \alpha (A,B)} \sum_{l=1}^d  \|P_N h\|_{L_{e_l}^{A^\prime ,B^\prime} (I\times \R^d )}. \end{equation*}
\end{lem}
\begin{rmq}
...
\begin{aligned}[t]
&\|P_N f\|_{Y_N(I)}= \langle N\rangle^C \left(\|P_N f\|_{L_t^{\frac{2(s+d)}{d}} L_x^{\frac{2(s+d)}{s}} (I\times \R^d)} +\|P_N f\|_{L_t^{\frac{2(s+d)}{s}} L_x^{\frac{2(s+d)}{s}}(I\times \R^d) } \right)
\\ & \qquad +\langle N\rangle^A \sum_{l=1}^d \|P_{N,e_l} P_N f \|_{L^{\frac{4}{\varepsilon},\frac{4}{2-\varepsilon}}_{e_l}(I\times \R^d) } +N^{-B} \sum_{l=1}^d \|P_N f\|_{L^{\frac{4}{2-\varepsilon},\frac{4}{\varepsilon}}_{e_l} (I\times \R^d) },  
\end{aligned}
\end{equation*}
...
\begin{equation*}N^{\beta +\alpha} \|P_{N_1}F_1 P_{N_2} v_2 P_{N_3} v_3\|_{L^{\frac{4}{4-\varepsilon} ,\frac{4}{2+\varepsilon}}_e} \lesssim  
N^{\beta +\alpha} \|P_{N_1}F_1 P_{N_2} v_2\|_{L^{2,2}_e}  \|P_{N_3} v_3\|_{L^{\frac{4}{2-\varepsilon} ,\frac{4}{\varepsilon}}_e}. \end{equation*}
...
Proceeding as in Term (6), with $(P_{N_2}v_2, P_{N_3}v_3)$ replaced by \\%%!
$(P_{N_3}v_3, P_{N_2}F_2)$ and $\theta_1$ by $\theta_2$ we have
\begin{multline*}
...
Proceeding as in Term (7), with $(P_{N_3}v_3, P_{N_2}F_2)$ replaced by \\%%!
$(P_{N_2}v_2, P_{N_3} F_3)$  we have
\begin{multline*}
...
\end{aligned}
\end{multline*}
Finally, we estimate the last component of $Y_N (\R)$. We consider two cases depending on the size of $N$ and we first  assume that $N$ is large. Then, from triangle inequality, interpolation,  Lemma \ref{lemIO}, and Lemma \ref{lemmaIK}, it follows for any $p \geq \max (\frac{4}{2-\varepsilon},\frac{4}{\varepsilon})$
\begin{multline*}
...
\begin{funding}
The first author was supported by FCT - Funda\c c\~ao para a Ci\^encia e a Tecnologia, under the project: UIDB/04561/2020, J. F. was partly supported by grant NSF-DMS-1816408\end{funding}
...
\newblock On the probabilistic {C}auchy theory of the cubic nonlinear
  {S}chr\"{o}dinger equation on {$\mathbb{R}^d$}, {$d\geq3$}.
...
\newblock Higher order expansions for the probabilistic local {C}auchy theory
  of the cubic nonlinear {S}chr\"{o}dinger equation on {$\mathbb{R}^3$}.
...
\newblock Global well-posedness of the energy-critical nonlinear
  {S}chr\"{o}dinger equation with small initial data in {$H^1(\mathbb{T}^3)$}.
...
\newblock Global well-posedness and scattering for the defocusing
  energy-critical nonlinear {S}chr\"{o}dinger equation in {$\mathbb{R}^{1+4}$}.
\newblock {\em Amer. J. Math.}, 129(1):1--60, 2007.

\bibitem{spitz2021almost}
Martin Spitz.
...
\newblock {\em arXiv preprint arXiv:2110.11648 }, 2021.
...
% LDPC codes from latin squares free of small trapping sets. 2010, \arxiv{1008.4177}
